...
#pragma omp parallel 
	...
	if (maxtkrate)
	{
		...
		printf("\nMAX VALID TIME STEP FOR TEMPERATURE %e seconds\n",tktimestep);
	}
	/* Check timestep */
	...
	do
	{
			...
			for (m2=0;m2<ynumy;m2++)
		{
			...
			if(!bondm[mcmax1])
			{
				...
				heaterr(m1,m2,0,wwn,wwi);
				/* Thermal conductivity Equation koef calc add */
				heaterr(m1,m2,1,wwn,wwi);

				/* Add matrix */
				gausmat4_add(mcmax,0,wwn,wwi);
			}
			else	
			{
				/* Add vX Simple Boundary */
				tbonderr(mcmax,0,wwn,wwi);

				/* Rescale coefficients */
				for (n1=0;n1<=wwn[0];n1++) wwi[n1]/=timestep;
		...
		/* Solve Matrix */
		start=omp_get_wtime();
		gausmat4_solve(nodenum,0);
		printf("===================================\n");
		printf("Time taken for solving temperature equations = %e s ",omp_get_wtime()-start);
		/* End Solve Matrix */
		...
		{
			printf("\n---> KRUG %2d \n",m0+1);
			printf("THERMAL STEP = %e s    THERMAL TIME = %e s \n",timestep,tktimesum);
...
#pragma omp for schedule(runtime)
		for (m3=0;m3<marknum;m3++) 
		{
			...
			if (markx[m3]>0 && marky[m3]>0 && markx[m3]<xsize && marky[m3]<ysize && markt[m3]>=50 && markt[m3]<100)
			{
				...
				interpolate_temperature(markx[m3],marky[m3],&TK,&TK2);
				markk[m3]=TK;
			}
		}
...
#pragma omp for schedule(runtime)
		for (m3=0;m3<=marknum;m3++) 
		{
			...
			if (markx[m3]>0 && marky[m3]>0 && markx[m3]<xsize && marky[m3]<ysize && markt[m3]<50)
			{
				...
				int m10=nxsearch(markx[m3]);
				int m20=nysearch(marky[m3]);
				/* Interpolate temperature */
				interpolate_temperature(markx[m3],marky[m3],&TK,&TK2);
				...
				if(heatdif)
				{
					...
					if (celdx<0 || celdy<0 || celdx>1.0 ||celdy>1.0 || swt1<0) {printf("ERROR..: num=%ld x=%e y=%e celdx=%e celdy=%e swt1=%e",m3,markx[m3],marky[m3],celdx,celdy,swt1);}
					...
					celdx=(markx[m3]-gx[m10])/(gx[m10+1]-gx[m10]);
					celdy=(marky[m3]-gy[m20])/(gy[m20+1]-gy[m20]);
					if (celdx<0 || celdx>1 || celdy <0 || celdy>1 || swt1<0 ) {printf("ERROR..: %ld %e %e %e \n",m3,celdx,celdy,swt1);}
...
#pragma omp parallel for shared(marknum,markx,marky,markt,markk,tol1,xsize,ysize,xnumx,ynumy,gx,gy,tk,tk2) \
				...
				for (m3=0;m3<=marknum;m3++) 
		{
			...
			if (markx[m3]>0 && marky[m3]>0 && markx[m3]<xsize && marky[m3]<ysize && markt[m3]<50)
			{
				m10=nxsearch(markx[m3]);
				m20=nysearch(marky[m3]);
				
				/* Wt for nodes calc, add */
				celdx=(markx[m3]-gx[m10])/(gx[m10+1]-gx[m10]);
				celdy=(marky[m3]-gy[m20])/(gy[m20+1]-gy[m20]);
				if (celdx<0 || celdx>1 || celdy <0 || celdy>1) {printf("ERROR: %ld %e %e %e \n",m3,celdx,celdy,swt1);  getchar();}
	...
	if (printmod==10000) printf("Time for temperature (anti-)diffusion = %e s\n",omp_get_wtime()-start);
...
double heaterr(long int m1, long int m2, int ynerr,int wn[],double wi[])
	...
	/* ynerr - Calc mode: 0-Right, 1-Left, 2-Err */
{
	...
	switch(ynerr)
	{
		...
		wi[1]=1.0/timestep;
		/* - 1/Cp/ro*[dQx/dX] */
		qxcalc_add(m1-1,m2,1.0/xkf/cp[v[4]]/ro[v[4]],wn,wi);
		qxcalc_add(m1,m2,-1.0/xkf/cp[v[4]]/ro[v[4]],wn,wi);
		/* - 1/Cp/ro*[dQy/dY] */
		qycalc_add(m1,m2-1,1.0/ykf/cp[v[4]]/ro[v[4]],wn,wi);
		qycalc_add(m1,m2,-1.0/ykf/cp[v[4]]/ro[v[4]],wn,wi);
		/**/
		return 0;
...
/* Coefficients or value for Qx  Equation */ 
/* Qx=k(dT/dX) */
double qxcalc_add(long int m1, long int m2, double ynval, int wn[],double wi[])
	/* m1,m2 - node X,Y number */
	/* ynval - Val Qx Calc Y(0)/N (koefficient) */
{
	/* Qx horizontal position */
...
/* Qy=k(dT/dY) */
double qycalc_add(long int m1, long int m2, double ynval,int wn[],double wi[])
	/* m1,m2 - node X,Y number */
	/* ynval - Val Qy Calc Y(0)/N(koefficient) */
{
	/* Qx horizontal position */
...
/* T=CONST+KOEF*Tn */
double tbonderr(long int mcmax, int ynerr,int wn[],double wi[])
